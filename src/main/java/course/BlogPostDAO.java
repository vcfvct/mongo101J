...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        //
        Document post = postsCollection.find(eq("permalink", permalink)).first();
        String NUM_LIKES_KEY = "num_likes";
        List<Document> comments = (List<Document>)post.get("comments");
        Document docToUpdate = comments.get(ordinal);
        if(docToUpdate.get(NUM_LIKES_KEY) == null){
            docToUpdate.append(NUM_LIKES_KEY, 1);
        }else {
            int newNumber = (Integer)docToUpdate.get(NUM_LIKES_KEY) + 1;
            docToUpdate.put(NUM_LIKES_KEY, newNumber);
        }
        postsCollection.replaceOne(new Document("permalink", permalink), post);
    }
}
